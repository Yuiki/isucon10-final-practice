...
import {
  ReceiveBenchmarkJobRequest,
  ReceiveBenchmarkJobResponse,
} from "../../../proto/xsuportal/services/bench/receiving_pb"
...
import {
  ReportBenchmarkResultRequest,
  ReportBenchmarkResultResponse,
} from "../../../proto/xsuportal/services/bench/reporting_pb"
...
class BenchmarkQueueService implements BenchmarkQueue.IBenchmarkQueueServer {
  receiveBenchmarkJob(
    ...
    this.receiveBenchmarkJobPromise(call.request)
      .then((res) => {
        callback(null, res)
      })
      .catch((e) => {
  ...
  async receiveBenchmarkJobPromise(
    request: ReceiveBenchmarkJobRequest
  ): Promise<ReceiveBenchmarkJobResponse> {
    ...
    try {
      while (true) {
        ...
        const [gotLock] = await db.query(
          "SELECT 1 FROM `benchmark_jobs` WHERE `id` = ? AND `status` = ? FOR UPDATE",
          [job.id, BenchmarkJob.Status.PENDING]
        )
        if (gotLock != null) {
          ...
          await db.query(
            ...
            [
              BenchmarkJob.Status.SENT,
              handle,
              job.id,
              BenchmarkJob.Status.PENDING,
            ]
          )
          const [contest] = await db.query(
            "SELECT `contest_starts_at` FROM `contest_config` LIMIT 1"
          )
          jobHandle = {
            jobId: job.id,
            handle,
      ...
      if (jobHandle != null) {
        ...
        if (jobHandle.contestStartedAt) {
          const contestStartedAt = convertDateToTimestamp(
...
class BenchmarkReportService implements BenchmarkReport.IBenchmarkReportServer {
  ...
  async reportBenchmarkResultPromise(
    request: ReportBenchmarkResultRequest
  ): Promise<ReportBenchmarkResultResponse> {
    const db = await getDB()
    ...
    try {
      ...
      if (job == null) {
        await db.rollback()
        console.error(
          `Job not found: job_id=${request.getJobId()}, handle=${request
            .getHandle()
            .toString()}`
        )
        throw new Error(
          `Job ${request.getJobId()} not found or handle is wrong`
        )
      }
  ...
  async saveAsFinished(
    ...
    if (job.started_at == null || job.finished_at != null) {
      await db.rollback()
      throw new Error(
        `Job ${request.getJobId()} has already finished or has not started yet`
      )
    }
    ...
    `,
      [
        BenchmarkJob.Status.FINISHED,
        result?.getScoreBreakdown()?.getRaw(),
        result?.getScoreBreakdown()?.getDeduction(),
        result?.getPassed(),
        result?.getReason(),
        markedAt,
        request.getJobId(),
  ...
  async saveAsRunning(
    ...
    await db.query(
      `
        ...
        LIMIT 1
    `,
      [BenchmarkJob.Status.RUNNING, startedAt, request.getJobId()]
    )
  }
}
...
function formatDate(timestamp: Timestamp) {
  ...
  const datestr = `${date.getUTCFullYear()}-${String(
    ...
    date.getUTCSeconds()
  ).padStart(2, "0")}.${String(timestamp.getNanos() / 1000).padStart(6, "0")}`
  return datestr
}
...
function main() {
  ...
  server.addService<BenchmarkQueue.IBenchmarkQueueServer>(
    BenchmarkQueue.BenchmarkQueueService,
    new BenchmarkQueueService()
  )
  server.addService<BenchmarkReport.IBenchmarkReportServer>(
    BenchmarkReport.BenchmarkReportService,
