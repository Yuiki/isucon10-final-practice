...
import {
  InitializeRequest,
  InitializeResponse,
} from "../proto/xsuportal/services/admin/initialize_pb"
...
import {
  ...
  RespondClarificationRequest,
  RespondClarificationResponse,
} from "../proto/xsuportal/services/admin/clarifications_pb"
...
import {
  ListBenchmarkJobsResponse,
  EnqueueBenchmarkJobRequest as ContestantEnqueueBenchmarkJobRequest,
  EnqueueBenchmarkJobResponse as ContestantEnqueueBenchmarkJobResponse,
} from "../proto/xsuportal/services/contestant/benchmark_pb"
...
} from "../proto/xsuportal/services/contestant/notifications_pb"
import {
  SignupRequest,
  SignupResponse,
} from "../proto/xsuportal/services/contestant/signup_pb"
import {
  LoginRequest,
  LoginResponse,
} from "../proto/xsuportal/services/contestant/login_pb"
import { LogoutResponse } from "../proto/xsuportal/services/contestant/logout_pb"
import {
  GetRegistrationSessionResponse,
  UpdateRegistrationRequest,
  UpdateRegistrationResponse,
  DeleteRegistrationRequest,
  DeleteRegistrationResponse,
} from "../proto/xsuportal/services/registration/session_pb"
import {
  CreateTeamRequest,
  CreateTeamResponse,
} from "../proto/xsuportal/services/registration/create_team_pb"
import {
  JoinTeamRequest,
  JoinTeamResponse,
} from "../proto/xsuportal/services/registration/join_pb"
import {
...
const haltPb = (res: express.Response, code: number, humanMessage: string) => {
  res.contentType(
    "application/vnd.google.protobuf; proto=xsuportal.proto.Error"
  )
  res.status(code)
...
const getCurrentContestStatus = async (db: mysql.PoolConnection) => {
  ...
  if (
    ...
    fs.existsSync(DEBUG_CONTEST_STATUS_FILE_PATH)
  ) {
    contestStatusStr = fs
      .readFileSync(DEBUG_CONTEST_STATUS_FILE_PATH)
      .toString()
  }
...
const loginRequired: (
  ...
  db: mysql.PoolConnection,
  opts?: { team?: boolean; lock?: boolean }
) => Promise<boolean> = async (
  req,
  res,
  db,
  { team = true, lock = false } = {}
) => {
...
async function contestStatusRestricted(
  res: express.Response,
  statuses: Array<Contest.Status>,
  msg: string,
  db: mysql.PoolConnection
): Promise<boolean> {
...
async function getTeamResource(
  team: any,
  db: mysql.PoolConnection,
  detail: boolean = false,
  enableMembers: boolean = true,
  ...
  if (enableMembers) {
    ...
    members_pb = members
      ? members.map((m: any) => getContestantResource(m, memberDetail))
      : null
  }
...
async function getBenchmarkJobResource(job) {
  ...
  benchmarkJob.setCreatedAt(
    job.created_at ? convertDateToTimestamp(job.created_at) : null
  )
  benchmarkJob.setUpdatedAt(
    job.updated_at ? convertDateToTimestamp(job.updated_at) : null
  )
  benchmarkJob.setStartedAt(
    job.started_at ? convertDateToTimestamp(job.started_at) : null
  )
  benchmarkJob.setFinishedAt(
    job.finished_at ? convertDateToTimestamp(job.finished_at) : null
  )
  benchmarkJob.setResult(
    job.finished_at ? await getBenchmarkResultResource(job) : null
  )
  return benchmarkJob
}

async function getBenchmarkJobsResource(
  req: express.Request,
  db: mysql.PoolConnection,
  limit?: number
) {
  ...
  return await Promise.all<BenchmarkJob>(
    jobs.map((job) => getBenchmarkJobResource(job))
  )
}
...
async function getClarificationResource(
  clar: any,
  team: any,
  db: mysql.PoolConnection
) {
  ...
  clarificationResource.setCreatedAt(
    clar.created_at ? convertDateToTimestamp(clar.created_at) : null
  )
...
function getNotificationsResource(notifications: any[]) {
  return notifications.map((notification) => {
    const notificationResource = Notification.deserializeBinary(
      Buffer.from(notification.encoded_message, "base64")
    )
    notificationResource.setId(notification.id)
    notificationResource.setCreatedAt(
      notification.created_at
        ? convertDateToTimestamp(notification.created_at)
        : null
    )
...
function getContestResource(contest: any) {
  ...
  contestResource.setContestStartsAt(
    contest.contest_starts_at
      ? convertDateToTimestamp(contest.contest_starts_at)
      : null
  )
  contestResource.setContestEndsAt(
    contest.contest_ends_at
      ? convertDateToTimestamp(contest.contest_ends_at)
      : null
  )
  contestResource.setContestFreezesAt(
    contest.contest_freezes_at
      ? convertDateToTimestamp(contest.contest_freezes_at)
      : null
  )
  contestResource.setRegistrationOpenAt(
    contest.registration_open_at
      ? convertDateToTimestamp(contest.registration_open_at)
      : null
  )
...
async function getLeaderboardResource(
  db: mysql.PoolConnection,
  teamId: number = 0
) {
  ...
  for (const result of jobResults) {
    ...
    lbs.setStartedAt(
      result.started_at ? convertDateToTimestamp(result.started_at) : null
    )
    lbs.setMarkedAt(
      result.finished_at ? convertDateToTimestamp(result.finished_at) : null
    )
  ...
  for (const team of leaderboard) {
    ...
    if (team.best_score != null) {
      ...
      bs.setStartedAt(
        team.best_score_started_at
          ? convertDateToTimestamp(team.best_score_started_at)
          : null
      )
      bs.setMarkedAt(
        team.best_score_marked_at
          ? convertDateToTimestamp(team.best_score_marked_at)
          : null
      )
    ...
    if (team.latest_score != null) {
      ...
      ls.setStartedAt(
        team.latest_score_started_at
          ? convertDateToTimestamp(team.latest_score_started_at)
          : null
      )
      ls.setMarkedAt(
        team.latest_score_marked_at
          ? convertDateToTimestamp(team.latest_score_marked_at)
          : null
      )
...
const contestantPaths =
  "/contestant /contestant/benchmark_jobs /contestant/benchmark_jobs/:id /contestant/clarifications".split(
    " "
  )
...
const adminPaths =
  "/admin /admin/ /admin/clarifications /admin/clarifications/:id".split(" ")
...
app.post("/initialize", async (req, res, next) => {
  ...
  try {
    const request = InitializeRequest.deserializeBinary(
    ...
    await db.query(
      ...
      [
        ADMIN_ID,
        crypto.createHash("sha256").update(ADMIN_PASSWORD).copy().digest("hex"),
      ]
    )
    ...
    if (contest) {
      ...
      await db.query(
        ...
        [openAt.toDate(), startsAt.toDate(), freezeAt.toDate(), endsAt.toDate()]
      )
    } else {
      await db.query(`
      INSERT contest_config (registration_open_at, contest_starts_at, contest_freezes_at, contest_ends_at) VALUES
    ...
    benchmarkServer.setPort(
      parseInt(process.env.BENCHMARK_SERVER_PORT ?? "50051", 10)
    )
...
app.put("/api/admin/clarifications/:id", async (req, res, next) => {
  ...
  try {
    ...
    const request = RespondClarificationRequest.deserializeBinary(
      Uint8Array.from(req.body)
    )
    const [clarBefore] = await db.query(
    ...
    await db.query(
      `UPDATE clarifications SET
        ...
        WHERE id = ?
        LIMIT 1`,
      [request.getDisclose(), request.getAnswer(), req.params.id]
    )
    ...
    notifier.notifyClarificationAnswered(
      clar,
      db,
      wasAnswered && wasDisclosed == clar.disclosed
    )
...
app.get("/api/session", async (req, res, next) => {
  ...
  try {
    ...
    response.setContestant(
      contestant ? getContestantResource(contestant) : null
    )
  ...
  try {
    ...
    if (currentTeam) {
      ...
      let [t] = await db.query(
        ...
        [req.query.team_id, req.query.invite_token]
      )
      if (t == null) {
    ...
    if (team) {
      const teamResource = await getTeamResource(
        team,
        db,
        currentContestant?.id == currentTeam?.leader_id,
        true,
        false
      )
      response.setTeam(teamResource)
      response.setMemberInviteUrl(
        `/registration?team_id=${team.id}&invite_token=${team.invite_token}`
      )
...
app.post("/api/registration/team", async (req, res, next) => {
  const db = await getDB()
  try {
    const request = CreateTeamRequest.deserializeBinary(
      Uint8Array.from(req.body)
    )
    ...
    await db.query(
      ...
      [request.getName(), request.getIsStudent(), teamId, currentContestant.id]
    )
...
app.put("/api/registration", async (req, res, next) => {
  ...
  try {
    await db.beginTransaction()
    const request = UpdateRegistrationRequest.deserializeBinary(
      Uint8Array.from(req.body)
    )
    const currentContestant = await getCurrentContestant(req, db)
    const currentTeam = await getCurrentTeam(req, db)
...
app.post("/api/contestant/benchmark_jobs", async (req, res, next) => {
  ...
  try {
    await db.beginTransaction()
    const request = ContestantEnqueueBenchmarkJobRequest.deserializeBinary(
      Uint8Array.from(req.body)
    )
    const loginSuccess = await loginRequired(req, res, db)
    if (!loginSuccess) {
    ...
    const passRestricted = await contestStatusRestricted(
      res,
...
app.post("/api/contestant/clarifications", async (req, res, next) => {
  ...
  try {
    ...
    const request = RequestClarificationRequest.deserializeBinary(
      Uint8Array.from(req.body)
    )
    const currentTeam = await getCurrentTeam(req, db)
    await db.query(
    ...
    const [clar] = await db.query(
      "SELECT * FROM `clarifications` WHERE `id` = LAST_INSERT_ID() LIMIT 1"
    )
    const response = new RequestClarificationResponse()
    response.setClarification(
      await getClarificationResource(clar, currentTeam, db)
    )
    res.contentType(`application/vnd.google.protobuf`)
    res.end(Buffer.from(response.serializeBinary()))
  } catch (e) {
...
app.post("/api/contestant/push_subscriptions", async (req, res, next) => {
  ...
  try {
    ...
    const request = SubscribeNotificationRequest.deserializeBinary(
      Uint8Array.from(req.body)
    )
    const currentContestant = await getCurrentContestant(req, db)
    await db.query(
...
app.delete("/api/contestant/push_subscriptions", async (req, res, next) => {
  ...
  try {
    ...
    const request = UnsubscribeNotificationRequest.deserializeBinary(
      Uint8Array.from(req.body)
    )
    const currentContestant = await getCurrentContestant(req, db)
    await db.query(
